...
/*
 * Support for the experimental xx-zones-v1 protocol
 *
 * A zone corresponds to the usable area of an output (i.e. the output
 * minus exclusive layer-shell surfaces). We set its handle to the output
 * name, so all clients requesting a zone on the same output share it by
 * design. Compositors should make apps use the handle to share the zone
 * between cooperating processes, but this simplification makes the
 * implementation a lot easier and is what existing apps want.
 *
...
/*
 * Send frame_extents if they changed and position if it changed (or if 'force'
 * is set). Does nothing for items without a zone or with an unmapped view.
 */
static void
...
item_move_frame_to(struct zone_item *item, int fx, int fy)
{
	...
	if (view->pending_configure_serial) {
		/* move is applied once the client acknowledges the
		 * pending configure (or it times out). */
		...
		struct wl_resource *toplevel_resource)
{
	...
	if (!view) {
		/* The toplevel is already gone; item is inert */
